...
interface BusinessSimulatorProps {
  type?: "entrepreneur" | "corporate";
}

export const BusinessSimulator = ({ type = "entrepreneur" }: BusinessSimulatorProps) => {
  const [value, setValue] = useState(type === "corporate" ? 500000 : 50000);
  const [months, setMonths] = useState(type === "corporate" ? 36 : 24);
  const [purpose, setPurpose] = useState("capital");

  const maxValue = type === "corporate" ? 10000000 : 500000;
  const minValue = type === "corporate" ? 100000 : 5000;
  const maxMonths = type === "corporate" ? 120 : 48;
  const rate = type === "corporate" ? 1.49 : 2.49;
  ...
  const Icon = type === "corporate" ? Building2 : Briefcase;
  const title = type === "corporate" ? "Simule seu Crédito Corporativo" : "Simule seu Crédito Empresarial";
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        <div className="w-12 h-12 rounded-xl bg-atos-gold/20 flex items-center justify-center">
          <Icon className="h-6 w-6 text-atos-gold-dark" />
        </div>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <Slider
            ...
            min={minValue}
            max={maxValue}
            step={type === "corporate" ? 50000 : 5000}
            className="py-4"
          />
        ...
        <div className="pt-4 border-t border-border">
          <p className="text-sm text-muted-foreground mb-2">Taxa a partir de {rate}% a.m.</p>
        ...
        <p className="text-xs text-muted-foreground text-center">
          * Simulação com fins ilustrativos. Condições sujeitas à análise de crédito e garantias.
        </p>
      </div>
    </motion.div>
  );
